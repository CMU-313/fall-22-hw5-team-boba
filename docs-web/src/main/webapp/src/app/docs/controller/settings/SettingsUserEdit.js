 ...
 */
angular.module('docs').controller('SettingsUserEdit', ($scope, $dialog, $state, $stateParams, Restangular, $translate) => {
  /**
  ...
  $scope.remove = function () {
    ...
    $dialog.messageBox(title, msg, btns, (result) => {
      if (result === 'ok') {
        Restangular.one('user', $stateParams.username).remove().then(() => {
          $scope.loadUsers();
          $state.go('settings.user');
        }, (e) => {
  ...
  $scope.disableTotp = function () {
    ...
    $dialog.messageBox(title, msg, btns, (result) => {
      if (result === 'ok') {
        Restangular.one(`user/${$stateParams.username}/disable_totp`).post('').then(() => {
