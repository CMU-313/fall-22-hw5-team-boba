...
angular.module('ngProgress.provider', ['ngProgress.directive'])
  .service('ngProgress', () => ['$document', '$window', '$compile', '$rootScope', '$timeout', function ($document, $window, $compile, $rootScope, $timeout) {
    this.autoStyle = true;
    this.count = 0;
    this.height = '2px';
    ...
    this.start = function () {
      ...
      this.intervalCounterId = setInterval(() => {
        if (isNaN(self.count)) {
          ...
          self.count += (0.15 * (1 - Math.sqrt(self.remaining)) ** 2);
