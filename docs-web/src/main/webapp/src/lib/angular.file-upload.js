...
ngFileUpload.service('Upload', ['$parse', '$timeout', '$compile', '$q', 'UploadExif', function ($parse, $timeout, $compile, $q, UploadExif) {
  ...
  function resizeFile(files, attr, scope, ngModel) {
    ...
    if (resizeVal instanceof Function) {
      const defer = $q.defer();
      return resizeVal(files).then((p) => {
        resizeWithParams(p, files, attr, scope, ngModel).then((r) => {
          defer.resolve(r);
        }, (e) => {
          defer.reject(e);
        });
      ...
      const unwatch = scope.$watch(attr[directiveName], (file) => {
        ...
        if (file && file.type && file.type.search(getTagType(elem[0])) === 0
          ...
          if (size && Upload.isResizeSupported()) {
            ...
            Upload.resize(file, size).then((f) => {
              constructDataUrl(f);
            }, (e) => {
              throw e;
            });
...
ngFileUpload.service('UploadValidate', ['UploadDataUrl', '$q', '$timeout', function (UploadDataUrl, $q, $timeout) {
  ...
  upload.validatePattern = function (file, val) {
    ...
    const pattern = globStringToRegex(val); let
      valid = true;
  ...
  upload.validate = function (files, prevLength, ngModel, attr, scope) {
    ...
    validateSync('minSize', 'size.min', (file, val) => file.size + 0.1 >= upload.translateScalars(val));
    validateSync('maxSize', 'size.max', (file, val) => file.size - 0.1 <= upload.translateScalars(val));
    let totalSize = 0;
    validateSync('maxTotalSize', null, (file, val) => {
      totalSize += file.size;
    ...
    function validateAsync(name, validationName, type, asyncFn, fn) {
      function resolveResult(defer, file, val) {
        ...
        if (val != null) {
          asyncFn(file, val).then((d) => {
            resolveInternal(() => !fn(d, val));
          }, () => {
    ...
    promises.push(validateAsync(
      ...
      null,
      /image/,
      (file, val) => upload.emptyPromise(val),
      (r) => r,
    ));
    promises.push(validateAsync(
      'ratio',
      ...
      this.imageDimensions,
      (d, val) => {
        const split = val.toString().split(','); let
          valid = false;
        for (let i = 0; i < split.length; i++) {
    ...
    promises.push(validateAsync(
      ...
      /image/,
      this.imageDimensions,
      (d, val) => (d.width / d.height) - upload.ratioToFloat(val) < 0.0001,
    ));
    promises.push(validateAsync(
      'minRatio',
      'ratio.min',
    ...
    promises.push(validateAsync(
      ...
      /audio|video/,
      this.mediaDuration,
      (d, val) => d <= upload.translateScalars(val),
    ));
    promises.push(validateAsync(
      'minDuration',
      'duration.min',
    ...
    $q.all(promises).then(() => {
      ...
      validateSync('maxFiles', null, (file, val, i) => prevLength + i < val);
...
(function () {
  ...
  function linkDrop(scope, elem, attr, ngModel, $parse, $timeout, $window, upload, $http, $q) {
    ...
    elem[0].addEventListener('paste', (evt) => {
    ...
    function calculateDragOverClass(scope, attr, evt, callback) {
      const obj = attrGetter('ngfDragOverClass', scope, { $event: evt }); let
        dClass = 'dragover';
