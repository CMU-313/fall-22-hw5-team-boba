...
angular.module('ui.bootstrap.carousel', [])
  ...
  .controller('UibCarouselController', ['$scope', '$element', '$interval', '$timeout', '$animate', function ($scope, $element, $interval, $timeout, $animate) {
    ...
    self.addSlide = function (slide, element) {
      ...
      slides.sort((a, b) => +a.slide.index - +b.slide.index);
    ...
    function goNext(slide, index, direction) {
      ...
      if ($animate.enabled($element) && !$scope.$currentTransition
        ...
        $animate.on('addClass', slides[index].element, (element, phase) => {
          if (phase === 'close') {
            $scope.$currentTransition = null;
...
angular.module('ui.bootstrap.dateparser', [])
  ...
  .service('uibDateParser', ['$log', '$locale', 'dateFilter', 'orderByFilter', 'filterFilter', function ($log, $locale, dateFilter, orderByFilter, filterFilter) {
    ...
    this.init = function () {
      ...
      formatCodeToRegex = [
        ...
        },
        {
          key: 'yy',
          regex: '\\d{2}',
          apply(value) { value = +value; this.year = value < 69 ? value + 2000 : value + 1900; },
          formatter(date) {
            const _date = new Date();
            _date.setFullYear(Math.abs(date.getFullYear()));
        ...
        },
        {
          ...
          formatter(date) { return dateFilter(date, 'MMMM'); },
        },
        {
          key: 'MMM',
          regex: $locale.DATETIME_FORMATS.SHORTMONTH.join('|'),
          apply(value) { this.month = $locale.DATETIME_FORMATS.SHORTMONTH.indexOf(value); },
          formatter(date) { return dateFilter(date, 'MMM'); },
        },
        {
          key: 'MM',
          regex: '0[1-9]|1[0-2]',
          apply(value) { this.month = value - 1; },
          formatter(date) { return dateFilter(date, 'MM'); },
        },
        {
          key: 'M',
          regex: '[1-9]|1[0-2]',
          apply(value) { this.month = value - 1; },
          formatter(date) { return dateFilter(date, 'M'); },
        },
        {
          key: 'd!',
        ...
        },
        {
          ...
          regex: '[0-2][0-9]{1}|3[0-1]{1}',
          apply(value) { this.date = +value; },
          formatter(date) { return dateFilter(date, 'dd'); },
        },
        {
          key: 'd',
          regex: '[1-2]?[0-9]{1}|3[0-1]{1}',
          apply(value) { this.date = +value; },
          formatter(date) { return dateFilter(date, 'd'); },
        },
        {
          key: 'EEEE',
          regex: $locale.DATETIME_FORMATS.DAY.join('|'),
          formatter(date) { return dateFilter(date, 'EEEE'); },
        },
        {
          key: 'EEE',
          regex: $locale.DATETIME_FORMATS.SHORTDAY.join('|'),
          formatter(date) { return dateFilter(date, 'EEE'); },
        },
        {
          key: 'HH',
          regex: '(?:0|1)[0-9]|2[0-3]',
          apply(value) { this.hours = +value; },
          formatter(date) { return dateFilter(date, 'HH'); },
        },
        {
          key: 'hh',
          regex: '0[0-9]|1[0-2]',
        ...
        },
        {
          ...
          regex: '1?[0-9]|2[0-3]',
          apply(value) { this.hours = +value; },
          formatter(date) { return dateFilter(date, 'H'); },
        },
        {
          key: 'h',
          regex: '[0-9]|1[0-2]',
        ...
        },
        {
          ...
          formatter(date) { return dateFilter(date, 'sss'); },
        },
        {
          key: 'ss',
          regex: '[0-5][0-9]',
          apply(value) { this.seconds = +value; },
          formatter(date) { return dateFilter(date, 'ss'); },
        },
        {
          key: 's',
          regex: '[0-9]|[1-5][0-9]',
          apply(value) { this.seconds = +value; },
          formatter(date) { return dateFilter(date, 's'); },
        },
        {
          key: 'a',
          ...
          formatter(date) { return dateFilter(date, 'a'); },
        },
        {
          key: 'Z',
          regex: '[+-]\\d{4}',
        ...
        },
        {
          key: 'ww',
          regex: '[0-4][0-9]|5[0-3]',
          formatter(date) { return dateFilter(date, 'ww'); },
        },
        {
          key: 'w',
          regex: '[0-9]|[1-4][0-9]|5[0-3]',
          formatter(date) { return dateFilter(date, 'w'); },
        },
        {
          key: 'GGGG',
          regex: $locale.DATETIME_FORMATS.ERANAMES.join('|').replace(/\s/g, '\\s'),
          formatter(date) { return dateFilter(date, 'GGGG'); },
        },
        {
          key: 'GGG',
          regex: $locale.DATETIME_FORMATS.ERAS.join('|'),
          formatter(date) { return dateFilter(date, 'GGG'); },
        },
        {
          key: 'GG',
          regex: $locale.DATETIME_FORMATS.ERAS.join('|'),
          formatter(date) { return dateFilter(date, 'GG'); },
        },
        {
          key: 'G',
          regex: $locale.DATETIME_FORMATS.ERAS.join('|'),
          formatter(date) { return dateFilter(date, 'G'); },
        },
      ];
      ...
      if (angular.version.major >= 1 && angular.version.minor > 4) {
        formatCodeToRegex.push({
          ...
          apply(value) { this.month = $locale.DATETIME_FORMATS.STANDALONEMONTH.indexOf(value); },
          formatter(date) { return dateFilter(date, 'LLLL'); },
        });
      }
    };
    ...
    function getFormatCodeToRegex(key) {
      return filterFilter(formatCodeToRegex, { key }, true)[0];
    }
    ...
    this.filter = function (date, format) {
      ...
      return formatters.reduce((str, formatter) => str + formatter(date), '');
    };
    ...
    function ($scope, $element, $attrs, $parse, $interpolate, $locale, $log, dateFilter, datepickerConfig, $datepickerLiteralWarning, $datepickerSuppressError, dateParser) {
      ...
      [
        ...
        switch (key) {
          ...
          case 'maxDate':
          case 'minDate':
            $scope.$watch(`datepickerOptions.${key}`, (value) => {
              if (value) {
          ...
          case 'minMode':
            if ($scope.datepickerOptions[key]) {
              $scope.$watch(() => $scope.datepickerOptions[key], (value) => {
      ...
      this.init = function (ngModelCtrl_) {
        ...
        if ($scope.datepickerOptions.initDate) {
          ...
          $scope.$watch('datepickerOptions.initDate', (initDate) => {
      ...
      this.isDisabled = function (date) {
        return $scope.disabled
          ...
          || $scope.dateDisabled && $scope.dateDisabled({ date, mode: $scope.datepickerMode });
      };
      ...
      function extractOptions(ngModelCtrl) {
        ...
        if (angular.version.minor < 6) { // in angular < 1.6 $options could be missing
          ...
          ngModelOptions = ngModelCtrl.$options // start with a ModelOptions instance
            ...
            .createChild({ timezone }); // to keep from squashing a non-null value
        }
...
angular.module('ui.bootstrap.dropdown', ['ui.bootstrap.multiMap', 'ui.bootstrap.position'])
  ...
  .service('uibDropdownService', ['$document', '$rootScope', '$$multiMap', function ($document, $rootScope, $$multiMap) {
    ...
    this.isOnlyOpen = function (dropdownScope, appendTo) {
      ...
      if (openedDropdowns) {
        const openDropdown = openedDropdowns.reduce((toClose, dropdown) => {
    ...
    this.open = function (dropdownScope, element, appendTo) {
      ...
      const openedDropdowns = openedContainers.get(appendTo);
      if (openedDropdowns) {
        const openedScopes = openedDropdowns.map((dropdown) => dropdown.scope);
    ...
    this.close = function (dropdownScope, element, appendTo) {
      ...
      const openedDropdowns = openedContainers.get(appendTo);
      if (openedDropdowns) {
        const dropdownToClose = openedDropdowns.reduce((toClose, dropdown) => {
          if (dropdown.scope === dropdownScope) {
            return dropdown;
          }
  ...
  .factory('$uibModalStack', ['$animate', '$animateCss', '$document',
    ...
    function ($animate, $animateCss, $document, $compile, $rootScope, $q, $$multiMap, $$stackedMap, $uibPosition) {
        ...
        if (modal) {
          switch (evt.which) {
            ...
            case 9: {
              ...
              } else if ($modalStack.isFocusInLastItem(evt, list)) {
      ...
      $modalStack.open = function (modalInstance, modal) {
        ...
        function applyAriaHidden(el) {
          ...
          function getSiblings(el) {
            ...
            return Array.prototype.filter.call(children, (child) => child !== el[0]);
          }
        }
      };
      ...
      $modalStack.isModalFocused = function (evt, modalWindow) {
        if (evt && modalWindow) {
          const { modalDomEl } = modalWindow.value;
      ...
      $modalStack.loadFocusElementList = function (modalWindow) {
        if (modalWindow) {
          ...
          if (modalDomE1 && modalDomE1.length) {
            ...
            return elements
              ? Array.prototype.filter.call(elements, (element) => isVisible(element)) : elements;
          }
        }
      };
          ...
          $modal.open = function (modalOptions) {
            ...
            samePromise = promiseChain = $q.all([promiseChain])
              ...
              .then((tplAndVars) => {
                ...
                const component = {};
                let ctrlInstance; let ctrlInstantiate; const
                  ctrlLocals = {};
...
angular.module('ui.bootstrap.pagination', ['ui.bootstrap.paging', 'ui.bootstrap.tabindex'])
  .controller('UibPaginationController', ['$scope', '$attrs', '$parse', 'uibPaging', 'uibPaginationConfig', function ($scope, $attrs, $parse, uibPaging, uibPaginationConfig) {
    ...
    const pageLabel = angular.isDefined($attrs.pageLabel) ? function (idx) { return $scope.$parent.$eval($attrs.pageLabel, { $page: idx }); } : angular.identity;
...
angular.module('ui.bootstrap.timepicker', [])
  ...
  .controller('UibTimepickerController', ['$scope', '$element', '$attrs', '$parse', '$log', '$locale', 'uibTimepickerConfig', function ($scope, $element, $attrs, $parse, $log, $locale, timepickerConfig) {
    let hoursModelCtrl; let minutesModelCtrl; let
      secondsModelCtrl;
    ...
    this.setupMousewheelEvents = function (hoursInputEl, minutesInputEl, secondsInputEl) {
      ...
      hoursInputEl.on('mousewheel wheel', (e) => {
        if (!disabled) {
      ...
      minutesInputEl.on('mousewheel wheel', (e) => {
        if (!disabled) {
      ...
      secondsInputEl.on('mousewheel wheel', (e) => {
        if (!disabled) {
 ...
 */
  ...
  .controller('UibTypeaheadController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$q', '$timeout', '$document', '$window', '$rootScope', '$$debounce', '$uibPosition', 'uibTypeaheadParser',
    function (originalScope, element, attrs, $compile, $parse, $q, $timeout, $document, $window, $rootScope, $$debounce, $position, typeaheadParser) {
      ...
      originalScope.$watch(attrs.typeaheadMinLength, (newVal) => {
...
angular.module('ui.bootstrap.carousel').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibCarouselCss && angular.element(document).find('head').prepend('<style type="text/css">.ng-animate.item:not(.left):not(.right){-webkit-transition:0s ease-in-out left;transition:0s ease-in-out left}</style>'); angular.$$uibCarouselCss = true; });
angular.module('ui.bootstrap.datepicker').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibDatepickerCss && angular.element(document).find('head').prepend('<style type="text/css">.uib-datepicker .uib-title{width:100%;}.uib-day button,.uib-month button,.uib-year button{min-width:100%;}.uib-left,.uib-right{width:100%}</style>'); angular.$$uibDatepickerCss = true; });
angular.module('ui.bootstrap.position').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibPositionCss && angular.element(document).find('head').prepend('<style type="text/css">.uib-position-measure{display:block !important;visibility:hidden !important;position:absolute !important;top:-9999px !important;left:-9999px !important;}.uib-position-scrollbar-measure{position:absolute !important;top:-9999px !important;width:50px !important;height:50px !important;overflow:scroll !important;}.uib-position-body-scrollbar-measure{overflow:scroll !important;}</style>'); angular.$$uibPositionCss = true; });
angular.module('ui.bootstrap.datepickerPopup').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibDatepickerpopupCss && angular.element(document).find('head').prepend('<style type="text/css">.uib-datepicker-popup.dropdown-menu{display:block;float:none;margin:0;}.uib-button-bar{padding:10px 9px 2px;}</style>'); angular.$$uibDatepickerpopupCss = true; });
angular.module('ui.bootstrap.tooltip').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibTooltipCss && angular.element(document).find('head').prepend('<style type="text/css">[uib-tooltip-popup].tooltip.top-left > .tooltip-arrow,[uib-tooltip-popup].tooltip.top-right > .tooltip-arrow,[uib-tooltip-popup].tooltip.bottom-left > .tooltip-arrow,[uib-tooltip-popup].tooltip.bottom-right > .tooltip-arrow,[uib-tooltip-popup].tooltip.left-top > .tooltip-arrow,[uib-tooltip-popup].tooltip.left-bottom > .tooltip-arrow,[uib-tooltip-popup].tooltip.right-top > .tooltip-arrow,[uib-tooltip-popup].tooltip.right-bottom > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.top-left > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.top-right > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.bottom-left > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.bottom-right > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.left-top > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.left-bottom > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.right-top > .tooltip-arrow,[uib-tooltip-html-popup].tooltip.right-bottom > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.top-left > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.top-right > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.bottom-left > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.bottom-right > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.left-top > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.left-bottom > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.right-top > .tooltip-arrow,[uib-tooltip-template-popup].tooltip.right-bottom > .tooltip-arrow,[uib-popover-popup].popover.top-left > .arrow,[uib-popover-popup].popover.top-right > .arrow,[uib-popover-popup].popover.bottom-left > .arrow,[uib-popover-popup].popover.bottom-right > .arrow,[uib-popover-popup].popover.left-top > .arrow,[uib-popover-popup].popover.left-bottom > .arrow,[uib-popover-popup].popover.right-top > .arrow,[uib-popover-popup].popover.right-bottom > .arrow,[uib-popover-html-popup].popover.top-left > .arrow,[uib-popover-html-popup].popover.top-right > .arrow,[uib-popover-html-popup].popover.bottom-left > .arrow,[uib-popover-html-popup].popover.bottom-right > .arrow,[uib-popover-html-popup].popover.left-top > .arrow,[uib-popover-html-popup].popover.left-bottom > .arrow,[uib-popover-html-popup].popover.right-top > .arrow,[uib-popover-html-popup].popover.right-bottom > .arrow,[uib-popover-template-popup].popover.top-left > .arrow,[uib-popover-template-popup].popover.top-right > .arrow,[uib-popover-template-popup].popover.bottom-left > .arrow,[uib-popover-template-popup].popover.bottom-right > .arrow,[uib-popover-template-popup].popover.left-top > .arrow,[uib-popover-template-popup].popover.left-bottom > .arrow,[uib-popover-template-popup].popover.right-top > .arrow,[uib-popover-template-popup].popover.right-bottom > .arrow{top:auto;bottom:auto;left:auto;right:auto;margin:0;}[uib-popover-popup].popover,[uib-popover-html-popup].popover,[uib-popover-template-popup].popover{display:block !important;}</style>'); angular.$$uibTooltipCss = true; });
angular.module('ui.bootstrap.timepicker').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibTimepickerCss && angular.element(document).find('head').prepend('<style type="text/css">.uib-time input{width:50px;}</style>'); angular.$$uibTimepickerCss = true; });
angular.module('ui.bootstrap.typeahead').run(() => { !angular.$$csp().noInlineStyle && !angular.$$uibTypeaheadCss && angular.element(document).find('head').prepend('<style type="text/css">[uib-typeahead-popup].dropdown-menu{display:block;}</style>'); angular.$$uibTypeaheadCss = true; });

const dialogModule = angular.module('dialog', ['ui.bootstrap']);
...
dialogModule.factory('$dialog', ['$rootScope', '$uibModal', '$sce', function ($rootScope, $uibModal, $sce) {
  ...
  function modalOptions(templateUrl, controller, scope) {
    return { templateUrl, controller, scope };
  }
  ...
  return {
     ...
     */
    simpleDialog(templateUrl, controller, resultFn) {
    ...
    messageBox(title, message, buttons, resultFn) {
      const scope = angular.extend($rootScope.$new(false), { title, message: $sce.trustAsHtml(message), buttons });
      return dialog(modalOptions('template/messageBox/message.html', 'MessageBoxController', scope), (result) => {
